    ...
    function cargar() {
        ...
        $.post(page_root + "cargar", $("#formulario").values(), function(data) {
            ...
            for (var i = 0; i < r.length; i++)
            {
                if (r[i].menu)
                {
                    $("#menu_" + r[i].menu).attr("checked", true);
                }

                if (r[i].accion)
                {
                    $("#accion_" + r[i].accion).attr("checked", true);
                }
            ...
            $("#arbol").css("display", "");
            $("#div-acciones").show();
            alerta("Se han cargado los permisos del rol seleccionado.");
...
<div   style="width:570px; margin:auto">
    <form id="formulario">
        ...
        <div style="text-align:right; padding-bottom:3px; display:none" id="div-acciones">
            ...
            <input type="button" value="Recargar" onclick="cargar();" />
