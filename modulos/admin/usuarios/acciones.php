...
class Admin_usuario extends formulario_basico {
    function asignar() {
        $sql = "SELECT u.persona_id,
                    u.rol,
                    CONCAT_WS('',p.nombre,' ',p.apellido1,' ',
                    p.apellido2, ' [',p.identifica,']') as persona_nombre
                FROM admin_usuario u, persona p
    ...
    function getSQL() {
        ...
        if ($_GET["nombre"] != "" && $_GET["nombre"] != "NULL") {
            ...
            $sql_extra = " WHERE CONCAT_WS(' ', p.nombre,p.apellido1,p.apellido2) LIKE '%$t%' ";
        } 
        ...
        $sql = "SELECT
                    ...
                    r.nombre as rol
                FROM admin_usuario u, admin_rol r, persona p
                WHERE 
                        u.rol = r.id AND
                        u.persona_id=p.id
                        $s
