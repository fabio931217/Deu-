    ...
    function generar() {
        ...
        if (is_array($_POST['campo'])) {
            ...
            <tr> 
                ...
                <td class="tdc">:</td>
                <td class="tdd">
                    <select id="%2$s" name="%2$s" title="%1$s">
                        <?php
            ...
            <tr> 
                ...
                <td class="tdc">:</td>
                <td class="tdd">
                 <input type="text" name="%2$s" id="%2$s"  value="" title="%1$s" maxlength="%3$s"/>
                </td>            
            </tr>';
...
//Botones
                    $a = '<input type="button" name="accion" value="%1$s" onclick="%2$s()" class="accion-%2$s" />';
    ...
    function continuar()
    {
      $.post(page_root + "%1$s", $("#formulario").values(), function(data) {
        ...
        if (is_array($_POST['accion'])) {
            foreach ($_POST['accion'] as $i => $v) {
                ...
                if ($accion != "NULL" && $accion != "") {
                    $db->query("INSERT INTO `admin_accion` (`menu`, `accion`, `tipo_accion`, `archivo`,`requiere_permiso`) 
                    VALUES ('$menu', '$accion2', '$tipo', 'acciones.php','S') ");
                }
            }
        }
